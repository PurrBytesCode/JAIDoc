...
- The APP_ID is a UUID generated once per application instance — this can be used for request tracing or logging
  correlation.
...
- The startup filter is hardcoded to `spring.beans.instantiate` — this ensures the buffer only captures the most
  important startup metric.
