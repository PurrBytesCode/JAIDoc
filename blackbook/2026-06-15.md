...
- Mermaid: a `mermaid` fenced block becomes `<pre class="mermaid">` with an escaped source, and the head `<style>` and
  body `<script>` blocks are injected the first time a page gains a diagram.
